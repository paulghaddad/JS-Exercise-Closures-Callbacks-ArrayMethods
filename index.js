...
*/
function processLength(elements, fn) {
  return fn(elements.length)
}
...
*/
function processLastItem(stringList, fn) {
  return fn(stringList[stringList.length-1])
}
 ...
 * should return 1000.
*/
function processSum(numberList, fn) {
  const sum = numberList.reduce((acc, el) => acc + el, 0)
 ...
 * should return 1000.
*/
function processProduct(num1, num2, fn) {
  return fn(num1 * num2)
}
 ...
 * should return "sad".
*/
function processContains(item, elements, fn) {
  return fn(elements.includes(item))
}
 ...
 * should return 3.
*/
function processDuplicateFree(elements, fn) {
  const set = new Set(elements)

  return fn(Array.from(set))
}
...
function getFullNames(runners) {
  ...
  runners.forEach(function(runner) {
    const fullName = `${runner.last_name}, ${runner.first_name}`
    names.push(fullName)
  })
...
function tallyUpDonations(runners) {
  return runners.reduce((donations, runner) => donations + runner.donation, 0)
}
